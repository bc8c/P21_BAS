...
func ReadCodeInfo(contract *gateway.Contract, ID_code string ) string {
...
func ReadTokenInfo(contract *gateway.Contract, ID_token string ) string {
...
func CreateCodeInfo(contract *gateway.Contract, mCodeInfo CodeInfo ) string{
	...
	result, err := contract.SubmitTransaction("CreateCodeInfo", mCodeInfo.ID_code, mCodeInfo.DID_RO, mCodeInfo.DID_client, mCodeInfo.Scope, string(mCodeInfo.Hash_code[:]), mCodeInfo.Time_issueed, mCodeInfo.URI_Redirection, mCodeInfo.Condition, mCodeInfo.ID_token)
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}
...
func CreateTokenInfo(contract *gateway.Contract, mTokenInfo TokenInfo ) string{
	...
	log.Println("--> Submit Transaction: CreateCodeInfo, creates new CodeInfo")	
	result, err := contract.SubmitTransaction("CreateTokenInfo", mTokenInfo.ID_token, mTokenInfo.DID_RO, mTokenInfo.DID_client, mTokenInfo.Scope, string(mTokenInfo.Hash_token[:]), mTokenInfo.Time_issueed, mTokenInfo.Time_expiration, mTokenInfo.URI_Redirection, mTokenInfo.Condition)
	if err != nil {
		log.Fatalf("Failed to Submit transaction: %v", err)
	}
