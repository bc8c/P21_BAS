...
func main() {
	...
	log.Println("--> Evaluate Transaction: ReadCodeInfo, function returns all the current assets on the ledger")
	result, err = contract.EvaluateTransaction("ReadCodeInfo", "TestCID5")
	if err != nil {
		log.Fatalf("Failed to evaluate transaction: %v", err)
	}
	log.Println(string(result))
	...
	// // result, err = contract.SubmitTransaction("CreateTokenInfo", "TestCID2", "TestDIDRO2", "TESTDIDClient2", "all", "HashTest", "TestTime2", "TESTURI2", "Available", "TestTID2")
	// if err != nil {
	// 	log.Fatalf("Failed to Submit transaction: %v", err)
	// }
	// log.Println(string(result))
	...
	log.Println("--> Evaluate Transaction: ReadTokenInfo, function returns all the current assets on the ledger")
	result, err = contract.EvaluateTransaction("ReadTokenInfo", "TestTID5")
	if err != nil {
		log.Fatalf("Failed to evaluate transaction: %v", err)
	}
	log.Println(string(result))
