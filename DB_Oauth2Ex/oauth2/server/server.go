...
func (s *Server) HandleAuthorizeRequest(w http.ResponseWriter, r *http.Request) error {
	...
	var mQuery bytes.Buffer
	mQuery.WriteString("insert into mapTB (code, token) value ('")
	mQuery.WriteString(mCode)
	mQuery.WriteString("','')")

	log.Println(mQuery.String())

	result, err := db.Exec(mQuery.String())
	if err != nil {
		log.Fatal(err)
		return err
	}
	log.Println(result)
	log.Println("insert into mapTB (code, token) value ('", mCode, "','')")
...
func (s *Server) HandleTokenRequest(w http.ResponseWriter, r *http.Request) error {
	...
	// DY mod START
	// Record the token information in blockchain
	...
	var mQuery bytes.Buffer
	mQuery.WriteString("update mapTB set token = '")
	mQuery.WriteString(mToken)
	mQuery.WriteString("' where code = '")
	mQuery.WriteString(tgr.Code)
	mQuery.WriteString("'")
	log.Println(mQuery.String())
	...
	log.Println(result)
	log.Println("update mapTB set token = '", mToken, "' where code = '", tgr.Code, "'")
...
func (s *Server) ValidationBearerToken(r *http.Request) (oauth2.TokenInfo, error) {
	...
	var mQuery bytes.Buffer
	mQuery.WriteString("select code from mapTB where token = '")
	mQuery.WriteString(accessToken)
	mQuery.WriteString("'")
	log.Println(mQuery.String())

	var code string
	err = db.QueryRow(mQuery.String()).Scan(&code)
	if err != nil {
		log.Fatal(err)
	}
	log.Println(code)
	log.Println("select code from mapTB where token = '", accessToken, "'")
